...
int main()
{
    ifstream fin("calib.txt");//标定所用图像文件的路径
    ...
    Size board_size = Size(11,8);//标定板上每行、列的内角点数，一般地，行列数不要相同
    ...
    while(getline(fin,filename))//从文本文档中依次读取待标定图片名
    {
        ...
        else
        {
            ...
            cornerSubPix(view_gray,image_points_buf,Size(5,5),Size(-1,-1),TermCriteria(cv::TermCriteria::MAX_ITER + cv::TermCriteria::EPS, 40, 0.01));
           //亚像素精确化方法二
    ...
    Size square_size = Size(21,21);//设置棋盘格子的实际边长，单位为mm
